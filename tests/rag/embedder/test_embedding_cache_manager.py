...
def test_make_cache_key_returns_composite_format(cm):
    key = cm.make_cache_key(CHUNK_KEY, MODEL)
    assert key == f"{CHUNK_KEY}|{MODEL}"
...
def test_make_collection_name_correct_format(cm):
    name = cm.make_collection_name("investor.gov", 500, 50, "bge-small")
    assert name == "fin_investor_gov_c500_o50_bge-small"
...
def test_make_collection_name_truncates_to_63_chars(cm):
    name = cm.make_collection_name("very.long.domain.name.example.com", 500, 50, "bge-small")
    assert len(name) <= 63
...
def test_is_embedded_returns_false_for_empty_mapping(cm):
    assert cm.is_embedded(CHUNK_KEY, MODEL, {}) is False

def test_is_embedded_returns_false_when_status_not_success(cm):
    key = cm.make_cache_key(CHUNK_KEY, MODEL)
    assert cm.is_embedded(CHUNK_KEY, MODEL, {key: {"status": "failed"}}) is False

def test_is_embedded_returns_true_when_status_success(cm):
    key = cm.make_cache_key(CHUNK_KEY, MODEL)
    assert cm.is_embedded(CHUNK_KEY, MODEL, {key: {"status": "success"}}) is True
...
def test_get_pending_returns_key_in_chunk_but_not_embedding(cm):
    ...
    result = cm.get_pending_chunk_keys(chunk_mapping, {}, MODEL)
    assert CHUNK_KEY in result

def test_get_pending_excludes_already_embedded_key(cm):
    emb_key = cm.make_cache_key(CHUNK_KEY, MODEL)
    chunk_mapping = {CHUNK_KEY: {"status": "success"}}
    emb_mapping = {emb_key: {"status": "success"}}
    result = cm.get_pending_chunk_keys(chunk_mapping, emb_mapping, MODEL)
    assert result == []
...
def test_get_pending_includes_same_key_for_different_model(cm):
    ...
    chunk_mapping = {CHUNK_KEY: {"status": "success"}}
    emb_mapping = {emb_key: {"status": "success"}}
    result = cm.get_pending_chunk_keys(chunk_mapping, emb_mapping, other_model)
    assert CHUNK_KEY in result
