...
def route_after_router(state: AgentState) -> list[Send]:
    """Determine the next step(s) after the router node.

    Returns one Send per route in `route_decision.next`, fanning out to
    financial_concepts, realtime_quotes, and/or out_of_scope in parallel.
    Each Send carries the full current state, since the target nodes only
    read `messages`, `call_counts`, and `route_decision`.
    """
    routes = state["route"]
...
def build_graph(llm, collection_name: str | None = None):
    ...
    out_of_scope_node = make_out_of_scope_node()
    ...
    graph_builder.add_edge("out_of_scope", "synchronizer")
